...
namespace BDDSampleEx.Automation.AutoIt
{
    public static class AutoIt
    {
        public static void UploadFiles(params string[] files)
        {
            ...
            Thread.Sleep(1000);
            AutoItX3Declarations.AU3_ControlClick("[ACTIVE]", "Editing @ EX.UA - Mozilla Firefox",
                "[CLASS:GeckoFPSandboxChildWindow; INSTANCE:1]", "left", 1, 5, 5);
            Thread.Sleep(1000);

            AutoItX3Declarations.AU3_Send(filesPath.ToString(), 1);

            AutoItX3Declarations.AU3_ControlClick("[ACTIVE]", "Open",
                "[CLASS:Button; TEXT:&Open; INSTANCE:1]", "left", 1, 5, 5);
        }
    }
